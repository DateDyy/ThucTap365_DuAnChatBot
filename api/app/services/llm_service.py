...
def query_llm(user_query: str, k: int = 3) -> str:
    docs = safe_similarity_search(user_query, k=k)
    if not docs:
        return "⚠️ Dữ liệu tìm kiếm chưa đồng bộ hoặc không khả dụng. Hãy rebuild lại vector store."

    context = "\n\n".join([d.page_content for d in docs])
    prompt = rag_prompt.format(context=context, question=user_query)

    response = llm.invoke([
        {"role": "system", "content": "Bạn là một trợ lý AI chuyên về lập trình web. Hãy trả lời trực tiếp, rõ ràng và có ví dụ khi cần."},
        {"role": "user", "content": prompt}
    ])
    return response.content if hasattr(response, "content") else str(response)


def chat_llm(user_message: str, history: list, k: int = 3) -> str:
    ...
    response = llm.invoke([
        {"role": "system", "content": "Bạn là một trợ lý AI hỗ trợ hội thoại. Luôn trả lời thẳng vào câu hỏi, không vòng vo."},
        {"role": "user", "content": prompt}
    ])
    return response.content if hasattr(response, "content") else str(response)
